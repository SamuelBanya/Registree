...
** Todo
** In Progress
*** IN-PROGRESS Make frontend make Axios calls to C# backend
<2023-10-25 Wed 19:21>: I need to be able to force the frontend to make Axios based calls to the C# backend.

This will require me to utilize a new 'lib/api.js' file to handle these Axios calls in one spot in the NextJS folder structure.

<2023-11-01 Wed 21:42>: I can't figure out the following behavior:
- I am able to fill out the form which completes itself using 'Step2Form.tsx'
- This results in a 'Success' green snackbar being displayed, yet there is a 'Error creating registree!' error being thrown on 'RegistryWizard.tsx'.

I think this has to deal with 'api.js' being involved, and that I need to provide more error handling going forward to handle success or errors being thrown.
...
*** DONE Add a '.NET 7.0' backend as well as a related Postgresql database to the project
CLOSED: [2023-10-25 Wed 19:20]
...
<2023-10-25 Wed 19:19>: Now that the C# backend has been added with a related migration, the next task should be to actually force the frontend to make Axios calls to the related C# backend, which I think should be in its own separate branch / ticket.

With that in mind, this ticket is complete.
